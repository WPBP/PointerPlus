<?php
/**
 * Class QL_Pointer to facilitate creation of WP Pointers
 * @author QueryLoop
 */
...
class QL_Pointer {
	...
	function initial_pointers() {
		return apply_filters( 'queryloop_initial_pointers', array(
			// Pointers are added through the 'queryloop_initial_pointers' filter so you can
	...
	function admin_enqueue_assets() {
		...
		wp_enqueue_style( $this->prefix, $base_url . '/ql-pointer.css', array( 'wp-pointer' ) );

		wp_enqueue_script( $this->prefix, $base_url . '/ql-pointer.js', array( 'wp-pointer' ) );

		wp_localize_script( $this->prefix, 'ql_pointer', apply_filters( 'queryloop_easy_pointers_js_vars', $this->pointers ) );
	}
}